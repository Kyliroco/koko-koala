  ...
  <div id="exercice">
    ...
    <div>
      <ul class="flex" id="liste">
        <li class="input_legend" v-if="nbChiffresATrouver >= 1">
          <input class="blank arround1" /> Unités
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 2">
          <input class="blank arround1" /> Dizaines
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 3">
          <input class="blank arround1" /> Centaines
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 4">
          <input class="blank arround1" /> Milliers
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 5">
          <input class="blank arround1" /> Dizaines de Milliers
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 6">
          <input class="blank arround1" /> Centaines de Milliers
        </li>
        <li class="input_legend" v-if="nbChiffresATrouver >= 7">
          <input class="blank arround1" /> Milions
        </li>
      </ul>
    </div>
...
function main() {
  ...
  try {
    ...
    document.getElementById("nbQuestions").textContent =
      exoParam.value.nbQuestions;
    document.getElementById("questionEnCours").textContent =
      exoParam.value.questionEnCours;
    document.querySelector("progress").value =
      ((exoParam.value.questionEnCours - 1) * 100) / exoParam.value.nbQuestions;
  } catch (e) {
    return null;
  }
}
...
function choisirUnChiffre() {
  ...
  var texteListe = [
    ...
    "Milliers",
    "Dizaines de Miliers",
    "Centaines de Miliers",
    "Milions",
  ];
  chiffresATrouver = [];

  let chiffre =
    Math.floor(Math.random() * (limiteChiffresMax - limiteChiffresMin)) +
    limiteChiffresMin;
  nbChiffresATrouver.value = chiffre.toString().length;
  if (chiffre.toString().length) {
  }

  for (let i = 0; i < nbChiffresATrouver.value; i++) {
    chiffresATrouver.push(
      chiffre.toString().charAt(nbChiffresATrouver.value - 1 - i)
    );
  ...
  if (document.getElementById("points")) {
    ...
    for (let i = 0; i < chiffresATrouver.length; i++) {
      if (chiffresATrouver[i] == chiffresRemplis[i].value) {
        ...
        exoParam.value.points -= Math.round(
          exoParam.value.pointsParQuestions / nbChiffresATrouver.value
        );
        document.getElementById("count_icon").style.filter =
          "hue-rotate(-" +
          ((exoParam.value.pointsParQuestions * exoParam.value.nbQuestions -
            exoParam.value.points) *
            60) /
            (exoParam.value.pointsParQuestions * exoParam.value.nbQuestions) +
          "deg)";
      }
    ...
    if (!fin) {
      ...
      document.querySelector("progress").value =
        ((exoParam.value.questionEnCours + 1) * 100) /
        exoParam.value.nbQuestions;
    }
  }
}
